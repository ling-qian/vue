  ...
  <div class="wrapper-login">
    <div class="register_result">
      ...
      <transition name="fade">
        <div>
          <p class="h-40" v-show="!show_result"></p>
          <p class="active-result" v-show="show_result">{{result}}</p>
        </div>
      </transition>
    </div>
    <div class="login-container">
      ...
      <div class="login-header">
        <a href="javascript:;" :class="{active: loginway == 'login'}" @click="changeLogin('login')">&nbsp;&nbsp;&nbsp;&nbsp;登录&nbsp;&nbsp;&nbsp;&nbsp;·</a>
        <a href="javascript:;" :class="{active: loginway == 'register'}" @click="changeLogin('register')">&nbsp;&nbsp;&nbsp;&nbsp;注册&nbsp;&nbsp;&nbsp;&nbsp;</a>
      </div>
      <div class="login-input" v-if="loginway === 'login'">
        <form action="" name="formLogin" class="login-form" accept-charset="UTF-8" method="post">
          <div class="username">
          ...
          <div class="login-control">
            <span class="f-l checkbox" :class="{check: checked === true}" @click="checked = !checked">
      ...
      <div class="login-input" v-if="loginway === 'register'">
        <form action="" accept-charset="UTF-8" method="post" name="formRegister" class="form_register">
          <div class="email">
            <span class="span1"><i class="fa fa-envelope-o"></i></span>
            <input type="email" name="email" v-model="user.email" placeholder="邮件/电话号码" class="span2" id="email"/>
          </div>
          <div class="username">
              <span class="span1"><i class="fa fa-user"></i></span>
              <input type="text" name="nickname" v-model="user.nickname" placeholder="昵称" class="span2"/>
          </div>
          <div class="password">
            <span class="span1"><i class="fa fa-fa-unlock-alt"></i></span>
            <input type="password" name="password" v-model="user.password" required placeholder="密码" class="span2"/>
          </div>
          <div></div>
          <button class="register-btn btn" type="button" @click="doRegister()"><span>注册</span></button>
  ...
  export default {
    ...
    methods: {
      ...
      doLogin () {
        ...
        thid.$http.get(url).then(function (response){
          thid.dealResponse(response)
        })
      },
      // 处理响应
      dealResponse (response) {
        ...
        if (resData.status) {
          setTimeout(function () {
            ...
            selt.$store.dispatch('changeIslogin', true)
          },1000)
        } else {
          setTimeout(function () {
            self.show_result = false
